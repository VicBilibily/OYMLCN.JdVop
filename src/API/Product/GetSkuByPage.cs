...
namespace OYMLCN.JdVop
{
    public partial class JdVopApi
    {
        ...
        /// </param>
        public static async Task<RspResult<PageNumSku>> ProductGetSkuByPageAsync(string token, string pageNum, int pageNo = 1)
        {
        ...
        /// <inheritdoc cref="ProductGetSkuByPageAsync(string, string, int)"/>
        public async Task<RspResult<PageNumSku>> ProductGetSkuByPageAsync(string pageNum, int pageNo = 1)
            => await ProductGetSkuByPageAsync(this.AccessToken, pageNum, pageNo);
    }
}
