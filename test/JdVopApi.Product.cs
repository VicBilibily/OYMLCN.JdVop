...
namespace OYMLCN.JdVop.Test
{
    public partial class JdVopApiTest
    {
        ...
        public async Task ProductGetDetailTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetDetailAsync(null, default));

            var res = await ApiInstance.ProductGetDetailAsync(TestHelper.SkuNormal1, "isSelf,isJDLogistics");
            Assert.True(res.Success, res.ResultMessage);
        ...
        public async Task ProductGetDetailBookTest()
        {
            var res = await ApiInstance.ProductGetDetailAsync(TestHelper.SkuBook);
            Assert.True(res.Success, res.ResultMessage);
            var detail = res.Result;
            Assert.NotNull(detail);
        ...
        public async Task ProductGetDetailVideoTest()
        {
            var res = await ApiInstance.ProductGetDetailAsync(TestHelper.SkuVideo);
            Assert.True(res.Success, res.ResultMessage);
            var detail = res.Result;
            Assert.NotNull(detail);
        ...
        public async Task ProductSkuImageTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductSkuImageAsync(null, default));

            var sku = TestHelper.SkuNormal1;
            var res = await ApiInstance.ProductSkuImageAsync(sku);
            Assert.True(res.Success, res.ResultMessage);
        ...
        public async Task ProductSkuStateTestAsync()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductSkuStateAsync(null, default));

            var sku = TestHelper.SkuNormal1;
            var res = await ApiInstance.ProductSkuStateAsync(sku);
            Assert.True(res.Success, res.ResultMessage);
        ...
        public async Task ProductCheckAreaLimitTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductCheckAreaLimitAsync(null, default, 0, 0, 0, 0));

            var sku = TestHelper.SkuNormal1;
            var jdAddr = TestHelper.JDArress;
            var res = await ApiInstance.ProductCheckAreaLimitAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.True(res.Success, res.ResultMessage);
            var result = res.Result;
            Assert.NotNull(result);
            Assert.NotEmpty(result);
        ...
        public async Task ProductGetSkuGiftTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetSkuGiftAsync(null, default, 0, 0, 0, 0));

            var sku = TestHelper.SkuNormal1;
            var jdAddr = TestHelper.JDArress;
            var res = await ApiInstance.ProductGetSkuGiftAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.True(res.Success, res.ResultMessage);
            var result = res.Result;
            Assert.Null(result);

            sku = TestHelper.SkuHasGift;
            res = await ApiInstance.ProductGetSkuGiftAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.True(res.Success, res.ResultMessage);
            result = res.Result;
            Assert.NotNull(result);
        }
        ...
        public async Task ProductGetYanbaoSkuTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetYanbaoSkuAsync(null, default, 0, 0, 0, 0));

            var sku = TestHelper.SkuNormal1;
            var jdAddr = TestHelper.JDArress;

            var res = await ApiInstance.ProductGetYanbaoSkuAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            ...
            //res = await ApiInstance.ProductGetYanbaoSkuAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            //Assert.True(res.Success, res.ResultMessage);
            //var result = res.Result;
            //Assert.NotNull(result);
        }
        ...
        public async Task ProductGetIsCodTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetIsCodAsync(null, default, 0, 0, 0, 0));

            var sku = TestHelper.SkuNormal1;
            var jdAddr = TestHelper.JDArress;
            var result = await ApiInstance.ProductGetIsCodAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.True(result.Success);
            Assert.False(result.Result); // 不支持货到付款
            Assert.Equal(sku, result.SkuIds);

            sku = TestHelper.SkuNormal2;
            result = await ApiInstance.ProductGetIsCodAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.NotNull(result);
            Assert.True(result.Success);
        ...
        public async Task ProductGetBatchIsCodTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetBatchIsCodAsync(null, default, 0, 0, 0, 0));

            var sku = $"{TestHelper.SkuNormal1},{TestHelper.SkuNormal2}";
            var jdAddr = TestHelper.JDArress;
            var res = await ApiInstance.ProductGetBatchIsCodAsync(sku, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.True(res.Success, res.ResultMessage);
            var result = res.Result;
        ...
        public async Task ProductGetSimilarSkuTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetSimilarSkuAsync(null, default));

            var sku = TestHelper.SkuNormal1;
            var res = await ApiInstance.ProductGetSimilarSkuAsync(sku);
            Assert.True(res.Success, res.ResultMessage);
            var result = res.Result;
            Assert.NotNull(result);
            Assert.NotEmpty(result);
        ...
        public async Task ProductGetCategoryTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductGetCategoryAsync(null, default));

            var cate = "9987;653;655";
            var categories = cate.Split(';').Select(v => int.Parse(v));
            foreach (var category in categories)
            {
                var res = await ApiInstance.ProductGetCategoryAsync(category);
                Assert.True(res.Success, res.ResultMessage);
                var result = res.Result;
                Assert.NotNull(result);
        ...
        public async Task ProductTotalCheckTest()
        {
            await Assert.ThrowsAsync<ArgumentException>(() => JdVopApi.ProductTotalCheckAsync(null, default, 0, 0, 0, 0));
            ...
            var jdAddr = TestHelper.JDArress;
            var result = await ApiInstance.ProductTotalCheckAsync(skuNums, jdAddr.ProvinceId, jdAddr.CityId, jdAddr.CountyId, jdAddr.TownId.Value);
            Assert.True(result.Success);
